...
<path d="M192 128h64v64H192zM768 128h64v64H768z"/>
  ...
  openSetting(): void {
    ...
    renderRules(
      currentSettings.replacementRules || [
        { match: "^#任务 ", replace: "- [ ] " },
      ],
    );
    ...
    let getTemplateFns: Array<() => MemoTemplate> = [];

    const renderTemplates = (templates: MemoTemplate[]) => {
      templatesContainer.innerHTML = "";
      getTemplateFns = [];

      if (!templates || templates.length === 0) {
        templates = [{ id: generateId(), name: "", content: "" }];
      }
      ...
      templates.forEach((tpl, index) => {
        ...
        nameInput.placeholder = this.i18n.templateNamePlaceholder;
        nameInput.value = tpl.name;

        const contentInput = document.createElement("textarea");
        contentInput.className = "b3-text-field";
        contentInput.style.flex = "1";
        contentInput.style.minHeight = "32px";
        contentInput.style.resize = "vertical";
        contentInput.rows = 1;
        contentInput.placeholder = this.i18n.templateContentPlaceholder;
        ...
        addBtn.innerText = "+";
        addBtn.onclick = () => {
          const current = getTemplateFns.map((fn) => fn());
          current.splice(index + 1, 0, {
            id: generateId(),
            name: "",
            content: "",
          });
          renderTemplates(current);
        };
  ...
  private showQuickCaptureDialog(): void {
    ...
    const textarea = dialog.element.querySelector(
      "textarea",
