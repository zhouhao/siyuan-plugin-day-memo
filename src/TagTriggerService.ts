...
export class TagTriggerService {
  ...
  constructor(
    ...
    this.store = store;
    this.i18n = i18n;
    this.boundHandler = this.handleWsMessage.bind(this) as (
      event: CustomEvent,
    ) => void;
  ...
  private handleWsMessage(event: CustomEvent<TransactionDetail>): void {
    ...
    const tagName = settings.triggerTag || "to-memo";
    ...
    for (const tx of detail.data || []) {
      for (const op of tx.doOperations || []) {
        ...
        if (!this.containsTag(op.data, tagName)) continue;
  ...
  private containsTag(domString: string, tagName: string): boolean {
    // SiYuan represents tags as <span data-type="tag">tagname</span>
    if (domString.includes(`data-type="tag"`) && domString.includes(tagName)) {
      return true;
    }
    // Also match plain text #tag# format
    return domString.includes("#" + tagName + "#");
  }
  ...
  private processPendingBlocks(): void {
    ...
    // Match both #tag# (plain text) and bare tagname (from DOM textContent)
