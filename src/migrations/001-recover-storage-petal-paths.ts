...
// A short-lived bug in plugin development uploaded images to
// /storage/petal/.../assets/ instead of /assets/. SiYuan's HTTP server
// only serves static files from data/assets/, so those images rendered
// as broken icons. This migration finds any such paths in memo content,
// re-uploads each file from plugin storage to /assets/ (where SiYuan
// does serve them), and rewrites the memo content with the new path.
//
// Path format in stored memos has no leading slash and may contain spaces
// inside the filename, e.g.:
//   (storage/petal/siyuan-plugin-day-memo/assets/foo bar-1234567890-abc.jpg)
