...
export class MemoItem {
  ...
  private createElement(): HTMLElement {
    ...
    if (this.memo.reminderAt && this.memo.reminderAt > Date.now()) {
      ...
      reminder.className =
        "day-memo__badge day-memo__badge--reminder b3-tooltips b3-tooltips__s";
      reminder.innerHTML = "🔔";
      reminder.setAttribute(
        "aria-label",
        new Date(this.memo.reminderAt).toLocaleString(),
      );
    ...
    content.querySelectorAll(".day-memo__checkbox").forEach((checkbox) => {
      checkbox.addEventListener("change", (e) => {
        e.stopPropagation();
        const idx = parseInt(
          (checkbox as HTMLElement).dataset.checkIndex || "0",
          10,
        );
    ...
    const dailyNoteBtn = this.createActionBtn(
      "📅",
      this.i18n.addToDailyNote || "Add to Daily Note",
      () => this.callbacks.onAddToDailyNote?.(this.memo),
    );
    const annotateBtn = this.createActionBtn(
      "💬",
  ...
  private showContextMenu(x: number, y: number): void {
    ...
    const hasReminder =
      this.memo.reminderAt && this.memo.reminderAt > Date.now();
    menu.addItem({
      icon: "iconClock",
      label: hasReminder
  ...
  private createAnnotationLinks(): HTMLElement | null {
    ...
    const hasAnnotations =
      this.memo.annotations && this.memo.annotations.length > 0;
    ...
    if (hasSource) {
      ...
      link.className =
        "day-memo__annotation-link day-memo__annotation-link--source";
      link.href = "#";
      const preview = sourceMemo
        ? sourceMemo.content.length > 40
          ? sourceMemo.content.substring(0, 40) + "..."
          : sourceMemo.content
        : this.memo.annotationOf!;
      link.innerHTML = `<span class="day-memo__annotation-link-icon">↩</span><span class="day-memo__annotation-link-label">${escapeHtml(this.i18n.viewSource || "View Source")}</span><span class="day-memo__annotation-link-preview">${escapeHtml(preview)}</span>`;
    ...
    if (hasAnnotations) {
      for (const annotationId of this.memo.annotations!) {
        ...
        const link = document.createElement("a");
        link.className =
          "day-memo__annotation-link day-memo__annotation-link--annotations";
        link.href = "#";
        const contentPreview = annotationMemo
          ? annotationMemo.content.length > 60
            ? annotationMemo.content.substring(0, 60) + "..."
            : annotationMemo.content
          : annotationId;
        link.innerHTML = `<span class="day-memo__annotation-link-icon">💬</span><span class="day-memo__annotation-link-preview">${escapeHtml(contentPreview)}</span>`;
        link.addEventListener("click", (e) => {
          e.preventDefault();
          e.stopPropagation();
