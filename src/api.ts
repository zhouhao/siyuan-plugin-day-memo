...
export async function lsNotebooks(): Promise<Notebook[]> {
    const data = await request("/api/notebook/lsNotebooks") as { notebooks: Notebook[] };
    return data.notebooks;
}

export async function getNotebookConf(notebook: string): Promise<{ dailyNoteSavePath: string }> {
    const data = await request("/api/notebook/getNotebookConf", { notebook }) as { conf: { dailyNoteSavePath: string } };
    return data.conf;
}
...
export async function createDocWithMd(notebook: string, path: string, markdown: string): Promise<string> {
    const data = await request("/api/filetree/createDocWithMd", { notebook, path, markdown });
    return data as string;
}
