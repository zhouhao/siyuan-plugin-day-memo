...
- **Markdown 支持** — 粗体、斜体、代码、链接等，内联渲染
- **标签系统** — 使用 `#标签` 语法组织便签，自动提取并在侧栏标签云中显示
- **日历视图** — 真实月历，支持 `‹` `›` 翻月，按便签密度显示热力图着色，点击日期筛选
- **时间线视图** — 便签按日期分组，最新的在前
- **搜索** — 在侧栏中全文搜索所有便签
...
4. 使用过滤标签页（全部 / 已置顶 / 已归档）切换视图
5. 点击日历上的日期，筛选该天的便签
6. 点击侧栏中的标签按钮按标签过滤
7. 鼠标悬停在便签上可以看到编辑 / 置顶 / 归档 / 删除操作
...
├── utils.ts              # 工具函数：ID 生成、标签提取、Markdown→HTML、日期
├── api.ts                # 思源内核 API 封装
...
└── components/
    ...
    ├── Heatmap.ts        # 月历组件（导航与热力图）
    ├── MemoEditor.ts     # 编辑器（文本框 + 保存/取消 + 编辑模式）
